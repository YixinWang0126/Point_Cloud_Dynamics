...
PARTITIONED_NETWORKS_SRC = os.path.join(ROOT, "tools", "partitioned_networks", "src")
if PARTITIONED_NETWORKS_SRC not in sys.path:
    sys.path.append(PARTITIONED_NETWORKS_SRC)
...
def TPOT_log(X1, X2,
    ...
    def _obj(pi_s, pi_f):
        ...
        obj_ot   = dot(C_pd, pi_f)
        obj_gw   = dot(eta(C1, C2, pi_s.sum(-1), pi_s.sum(0)) - C1 @ pi_s @ C2.T, pi_s)
        total    = beta*obj_coot + (alpha * obj_gw + (1-alpha)*obj_ot)
        return total, {"coot": beta*obj_coot, "ot": (1-alpha)*obj_ot, "gw": alpha*obj_gw}
    ...
    # ====== 先记录初始化状态（iter=0） ======
    obj, obj_terms = _obj(pi_s, pi_f)
    loss_history.append({"iter": 0,
                        "total": float(obj),
                        "geom": float(obj_terms["gw"]),     
                        "topo": float(obj_terms["ot"]),     
                        "hyper": float(obj_terms["coot"])})
    ...
    for it in range(iter):
        ...
        # ====== 本轮评估并记录（iter 从 1 开始累计）======
        obj_new, obj_new_terms = _obj(pi_s, pi_f)
        loss_history.append({"iter": it+1,
                            "total": float(obj_new),
                            "geom": float(obj_new_terms["gw"]),
                            "topo": float(obj_new_terms["ot"]),
                            "hyper": float(obj_new_terms["coot"])})

        # 收敛检测
        if (abs(obj_new - obj)/obj < rel_tol) or (abs(obj_new - obj) < abs_tol):
            is_converged = True
            if verbose:
                print(f"is_converged, obj = {obj}, obj_new = {obj_new}")
            obj = obj_new
    ...
    # 维持原有第三个返回项（当前 obj 和分项），再额外返回 loss_history
    return pi_s, pi_f, _obj(pi_s, pi_f), loss_history
