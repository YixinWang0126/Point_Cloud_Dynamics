...
HYPERCOT_PATH = os.path.join(
    ROOT, "tools", "partitioned_networks", "tools", "HyperCOT"
)
if HYPERCOT_PATH not in sys.path:
    sys.path.append(HYPERCOT_PATH)
...
def process_hg(h, M, top_n=None):
    ...
    w = incidence(h, M, len(edge_list))
    
    return w, np.full((w.shape[0], ), 1/w.shape[0]), v

def incidence(g, M, N):
    A = np.zeros((M, N))
    edge_list = list(g.edges())  # 转换为列表
    for col_idx, edge_name in enumerate(edge_list):
        nodes = g.edges[edge_name]  # 获取节点集合
        node_indices = [int(node) - 1 for node in nodes]
        A[node_indices, col_idx] = 1
    return A
'''
def incidence(g, M, N):
    A = np.zeros((M, N))
